...
library(gocc)
hbpc_path <- system.file(file.path("templates", "hbpc", "texMemo.cls"), package="gocc")
tex_cls_name <- tools::file_path_sans_ext(hbpc_path)
@
...
\begin{itemize}
  ...
  \item The segments of each column compare the number without documentation to the number who had documentation from the LST template at any time between the first use of the LST template at \Sexpr{name}, and up to 30 days following each HBPC admission.
\end{itemize}
