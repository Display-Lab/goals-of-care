...
# Try to load data from build directory if it is not already present
  if(exists("rate_data") == FALSE || exists("category_data") == FALSE) {
    prj_filepath <- file.path(here::here(),"build","performance_hbpc.rdata")
...
<<plot1, include=TRUE, message=FALSE, echo=FALSE, warning=FALSE, fig.1p="fig:", fig.keep='all', fig.width = 6.5, fig.height = 3.5, fig.align='center', fig.show='hold'>>=
  ...
  title_txt <- paste("How many total newly admitted Veterans have a documented",
                     ...
                     paste("HBPC", timespan_text(plot_data$timepoint)),
                     sep="\n",collapse="")
...
<<plot2, include=TRUE, message=FALSE, echo=FALSE, warning=FALSE, fig.1p="fig:", fig.keep='all', fig.width = 7, fig.height = 3.5, fig.align='center', fig.show='hold'>>=
  ...
  # Format title of plot
  title_txt <- paste("During which visit were goals of care conversations documented?",
                     paste("HBPC", timespan_text(plot_data$timepoint)),
                     sep="\n",collapse="")
  ...
  ctgy_plot <- category_plot(plot_data,
    plot_title = title_txt,
    y_label = "Veterans Admitted",
    cat_labels = c("61-90 days", "31-60 days", "0-30 days", "Before admission")
  )
...
\begin{itemize}
  ...
  \item Each Veteran with a documented GoCC was counted only once, starting with "0-30 days", "31-60 days", "61-90 days", and finally "Before admission".
