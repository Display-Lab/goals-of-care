...
% Setup conditional to include latex content for tips and comparison sections
\newif\ifincludetips
...
\noindent Tips based on an assessment of your CLC:
<<clc_exp, include=include_tips, results="asis", echo=FALSE>>=
  cat("\\begin{itemize}")
  cat("", tips, sep="\n\\item ")
  cat("\\end{itemize}")
@
...
<< compare_data, include=FALSE, results="asis", echo=FALSE>>=
  ...
  plot_data <- all_plot_data %>%
    ...
    group_by(trtsp_1, timepoint) %>%
    mutate(rate = count/denominator,
           med = stats::median(rate, na.rm=T)) %>%
...
<<compare_plot, include=TRUE, message=FALSE, echo=FALSE, warning=FALSE, fig.1p="fig:", fig.keep='all', fig.width = 7, fig.height = 3, fig.align='center', fig.show='hold'>>=
  title_txt <- "% Newly admitted Veterans with completed LST template"
...
\begin{itemize}
<<contacts_list, include=TRUE, results="asis", echo=FALSE>>=
  cat("", contacts, sep="\n\\item[] ")
@
\end{itemize}\bigskip
