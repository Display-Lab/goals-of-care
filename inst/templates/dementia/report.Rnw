...
The following pages present data about the number of goals of care conversations (GoCCs) that were documented using the Life-Sustaining Treatment template for Veterans at the \Sexpr{name}. \bigskip
...
<<dimentia_table, include=TRUE, message=FALSE, echo=FALSE>>=
  ...
  summ_table_data <- dementia_summary_table_data(plot_data) %>%
    ...
    mutate(rgroup=case_when(
           ...
           evt = case_when(
             trtsp_1=="Long-Term Care Residents" & evt == "Total number pts" ~ "Total number residents",
             TRUE ~ evt), 
  ...
  knitr::kable(summ_table_data, format="latex", booktabs=T, caption="Veterans goals of care documentation") %>%
