...
# Try to load data from build directory if it is not already present
  if(exists("rate_data") == FALSE || exists("category_data") == FALSE) {
    prj_filepath <- file.path(here::here(),"build","performance_clc.rdata")
...
if( !exists("generate_rate_plot") ){ try_source(file.path(here::here(),"lib","rate_plot.r")) }

if( !exists("generate_category_plot") ){ try_source(file.path(here::here(),"lib","categories_plot.r")) }

# Get the config and store it in a variable.
config_path <- file.path(here::here(),"config","report_settings.yml")
...
\begin{itemize}
  ...
  \item The segments of each column compare the number without documentation to the number who had documentation from the LST template at any time between the first use of the LST template by \Sexpr{site$name}, and up to 30 days following each CLC admission.
\end{itemize}
...
<<plot2, include=TRUE, message=FALSE, echo=FALSE, warning=FALSE, fig.1p="fig:", fig.keep='all', fig.width = 7, fig.height = 3.5, fig.align='center', fig.show='hold'>>=
  ...
  # Format data for plotting
  plot_data <- category_plot_data(category_data)
  
  # This plot should only have data from a single recipient (site).
  plot_data <- filter(plot_data, id == selected_id)
  
  # Format title of plot
  title_txt <- paste("How close to the time of admission were goals of care",
                     "conversations documented?",
                     paste("CLC", timespan_text(plot_data$timepoint)),
                     sep="\n",collapse="")
  
  # Create plot
  ctgy_plot <- category_plot(plot_data,
    plot_title = title_txt,
    y_label = "Veterans Admitted",
