...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
<<plot1, fig=TRUE, include=TRUE, message=FALSE, echo=FALSE, warning=FALSE, fig.1p="fig:", fig.keep='all', fig.width = 6.5, fig.height = 3.5, fig.align='center', fig.show='hold'>>=

  library(dplyr)

  # Check for existence of hit_rate_plot_data data.frame
  # This will inidicate it's been loaded into the environment externally.
  # Otherwise, try to load it from the expected location relative to this script.
  if(exists("perf_data") == FALSE) {
    input_filepath <- file.path("..", "..", "build","perf.rdata")
    load(input_filepath)
  }
  ...
  plot_data <- transform_to_hit_rate_data(perf_data)
  
  # Manually specify a single identifier (currently location) to futher filter data.
  plot_data <- filter(plot_data, location == "607")
...
<<plot2, fig=TRUE, include=TRUE, message=FALSE, echo=FALSE, warning=FALSE, fig.1p="fig:", fig.keep='all', fig.width = 7, fig.height = 3.5, fig.align='center', fig.show='hold'>>=
