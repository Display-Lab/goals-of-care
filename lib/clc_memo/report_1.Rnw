...
#  the location ID in this case.
ids = levels(hit_rate_data$location)
write( paste("REPORT SELECTED_ID:", selected_id), stderr())
if( !exists("selected_id") ){ selected_id <- ids[1] }
  
@


\memoto{ \Sexpr{config$id_names[selected_id]} }
\memofrom{ \Sexpr{config$provider} }
