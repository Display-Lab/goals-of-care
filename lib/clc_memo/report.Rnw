...
On the reverse side of this page are feedback reports that present data about the completeness and timeliness with which clinicians document goals of care conversations for Veterans admitted to CLC at \Sexpr{config$id_names[[selected_id]]}.\bigskip
...
\begin{itemize}
  ...
  \item The segments of each column compare the number without documentation to the number who had documentation from the LST template at any time between the first use of the LST template by \Sexpr{config$id_names[[selected_id]]}, and up to 30 days following each Veteran's admission.
\end{itemize}
