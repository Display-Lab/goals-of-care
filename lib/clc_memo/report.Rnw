...
# Try to load data from build directory if it is not already present
  if(exists("rate_data") == FALSE || exists("category_data") == FALSE) {
    rel_filepath <- file.path("..", "..", "build","performance_clc.rdata")
    prj_filepath <- file.path("build","performance_clc.rdata")
...
config <- full_config$clc

# Use subset data by selected_identifier.
#  This is set in the parent environment or defaults to the first id found.
...
\memoto{ \Sexpr{config$id_names[[selected_id]]} CLC Providers and Staff }
...
<<plot2, include=TRUE, message=FALSE, echo=FALSE, warning=FALSE, fig.1p="fig:", fig.keep='all', fig.width = 7, fig.height = 3.5, fig.align='center', fig.show='hold'>>=
  ...
  category_plot <- generate_category_plot(plot_data,
    plot_title = "How close to the time of admission were goals of care \nconversations documented?",
    y_label = "Veterans Admitted",
...
\begin{itemize}
  ...
  \item Each Veteran with a documented goals of care conversation was counted only once, starting with "0 to 7 days after", then "8 to 30 days after", then "Before admission".
